...
//  OBD.OEM.NIO
//
//  NIO Inc. OEM extension. Catalogue + DTC overlay in
//  <c>catalogs/nio.json</c> + <c>catalogs/dtc-nio.json</c>.
//
...
type
  /// <summary>NIO OEM extension.</summary>
...
function TOBDOEMExtensionNIO.ApplicableToVIN(
  const VIN: string): Boolean;
begin
  Result := VINMatchesCatalog('nio.json', VIN);
end;
...
procedure TOBDOEMExtensionNIO.BuildExtendedCatalog(
  var CodingBlocks: TArray<TOBDOEMCodingBlock>;
  var Adaptations: TArray<TOBDOEMAdaptation>;
  var ActuatorTests: TArray<TOBDOEMActuatorTest>;
  var LivePIDs: TArray<TOBDOEMLivePID>;
...
begin
  MergeExtendedCatalogJSON('nio.json',
...
begin
  case DID of
    ...
    $F1A0, $F1A2:
      ...
      begin
        case DID of
          $F1A0: FieldName := 'nio_model_code';
          $F1A2: FieldName := 'nio_battery_swap_id';
        else
          FieldName := 'unknown';
        end;
        Result := Format('%s = "%s"',
