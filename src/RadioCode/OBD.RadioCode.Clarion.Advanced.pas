//------------------------------------------------------------------------------
// UNIT           : OBD.RadioCode.Clarion.Advanced.pas
// CONTENTS       : Clarion Radio Code Calculator - Advanced Multi-Variant
// VERSION        : 2.0
...
//------------------------------------------------------------------------------
unit OBD.RadioCode.Clarion.Advanced;
...
type
  /// <summary>
  ///   Clarion Advanced Radio Code Calculator with Multiple Algorithm Variants
  /// </summary>
  TOBDRadioCodeClarionAdvanced = class(TOBDRadioCode)
  private
...
constructor TOBDRadioCodeClarionAdvanced.Create;
begin
  inherited Create;
  FVariantManager := TRadioCodeVariantManager.Create('Clarion Advanced');
  InitializeVariants;
  FCurrentVariant := FVariantManager.GetDefaultVariant;
end;

destructor TOBDRadioCodeClarionAdvanced.Destroy;
begin
  FVariantManager.Free;
  inherited Destroy;
end;

procedure TOBDRadioCodeClarionAdvanced.InitializeVariants;
var
...
begin
  ...
  Variant := FVariantManager.AddVariant(
    'CLARION_EU_V1',
    'Clarion Europe V1 (1995-2005)',
    rcrEurope,
    1995, 2005,
    rcsvV1,
    True
  );
  Variant.RadioModels.Add('Clarion models');
  Variant.AlgorithmNotes := 'Basic algorithm for European market';
  
  // Europe V2 (2006-2015)
  Variant := FVariantManager.AddVariant(
    'CLARION_EU_V2',
    'Clarion Europe V2 (2006-2015)',
    rcrEurope,
    2006, 2015,
    rcsvV2
  );
...
function TOBDRadioCodeClarionAdvanced.CalculateV1(const Serial: string): string;
var
  Sanitized: string;
  Serial: Integer;
...
function TOBDRadioCodeClarionAdvanced.CalculateV2(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeClarionAdvanced.CalculateV3(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeClarionAdvanced.GetDescription: string;
begin
  Result := 'Advanced Clarion Radio Code Calculator with multiple algorithm variants';
end;

procedure TOBDRadioCodeClarionAdvanced.SetVariant(const VariantID: string);
var
  Variant: TRadioCodeVariant;
begin
...
procedure TOBDRadioCodeClarionAdvanced.SetVariant(const Region: TRadioCodeRegion; const ModelYear: Integer);
var
  Variant: TRadioCodeVariant;
begin
...
function TOBDRadioCodeClarionAdvanced.GetCurrentVariant: TRadioCodeVariant;
begin
  Result := FCurrentVariant;
end;

function TOBDRadioCodeClarionAdvanced.GetAvailableVariants: TRadioCodeVariantManager;
begin
  Result := FVariantManager;
end;

function TOBDRadioCodeClarionAdvanced.Validate(const Input: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
...
function TOBDRadioCodeClarionAdvanced.Calculate(const Input: string; var Output: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
  ...
  if (FCurrentVariant.VariantID = 'CLARION_EU_V1') or
     (FCurrentVariant.VariantID = 'CLARION_NA') then
    Output := CalculateV1(Sanitized)
  else if (FCurrentVariant.VariantID = 'CLARION_EU_V2') or
          (FCurrentVariant.VariantID = 'CLARION_ASIA') then
    Output := CalculateV2(Sanitized)
  else if FCurrentVariant.VariantID = 'CLARION_EU_V3' then
