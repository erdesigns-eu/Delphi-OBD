//------------------------------------------------------------------------------
// UNIT           : OBD.RadioCode.Visteon.Advanced.pas
// CONTENTS       : Visteon Radio Code Calculator - Advanced Multi-Variant
// VERSION        : 2.0
...
//------------------------------------------------------------------------------
unit OBD.RadioCode.Visteon.Advanced;
...
type
  /// <summary>
  ///   Visteon Advanced Radio Code Calculator with Multiple Algorithm Variants
  /// </summary>
  TOBDRadioCodeVisteonAdvanced = class(TOBDRadioCode)
  private
...
constructor TOBDRadioCodeVisteonAdvanced.Create;
begin
  inherited Create;
  FVariantManager := TRadioCodeVariantManager.Create('Visteon Advanced');
  InitializeVariants;
  FCurrentVariant := FVariantManager.GetDefaultVariant;
end;

destructor TOBDRadioCodeVisteonAdvanced.Destroy;
begin
  FVariantManager.Free;
  inherited Destroy;
end;

procedure TOBDRadioCodeVisteonAdvanced.InitializeVariants;
var
...
begin
  ...
  Variant := FVariantManager.AddVariant(
    'VISTEON_EU_V1',
    'Visteon Europe V1 (1995-2005)',
    rcrEurope,
    1995, 2005,
    rcsvV1,
    True
  );
  Variant.RadioModels.Add('Visteon models');
  Variant.AlgorithmNotes := 'Basic algorithm for European market';
  
  // Europe V2 (2006-2015)
  Variant := FVariantManager.AddVariant(
    'VISTEON_EU_V2',
    'Visteon Europe V2 (2006-2015)',
    rcrEurope,
    2006, 2015,
    rcsvV2
  );
...
function TOBDRadioCodeVisteonAdvanced.CalculateV1(const Serial: string): string;
var
  Sanitized: string;
  Serial: Integer;
...
function TOBDRadioCodeVisteonAdvanced.CalculateV2(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeVisteonAdvanced.CalculateV3(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeVisteonAdvanced.GetDescription: string;
begin
  Result := 'Advanced Visteon Radio Code Calculator with multiple algorithm variants';
end;

procedure TOBDRadioCodeVisteonAdvanced.SetVariant(const VariantID: string);
var
  Variant: TRadioCodeVariant;
begin
...
procedure TOBDRadioCodeVisteonAdvanced.SetVariant(const Region: TRadioCodeRegion; const ModelYear: Integer);
var
  Variant: TRadioCodeVariant;
begin
...
function TOBDRadioCodeVisteonAdvanced.GetCurrentVariant: TRadioCodeVariant;
begin
  Result := FCurrentVariant;
end;

function TOBDRadioCodeVisteonAdvanced.GetAvailableVariants: TRadioCodeVariantManager;
begin
  Result := FVariantManager;
end;

function TOBDRadioCodeVisteonAdvanced.Validate(const Input: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
...
function TOBDRadioCodeVisteonAdvanced.Calculate(const Input: string; var Output: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
  ...
  if (FCurrentVariant.VariantID = 'VISTEON_EU_V1') or
     (FCurrentVariant.VariantID = 'VISTEON_NA') then
    Output := CalculateV1(Sanitized)
  else if (FCurrentVariant.VariantID = 'VISTEON_EU_V2') or
          (FCurrentVariant.VariantID = 'VISTEON_ASIA') then
    Output := CalculateV2(Sanitized)
  else if FCurrentVariant.VariantID = 'VISTEON_EU_V3' then
