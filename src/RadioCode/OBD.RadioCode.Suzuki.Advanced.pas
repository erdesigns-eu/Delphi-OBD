//------------------------------------------------------------------------------
// UNIT           : OBD.RadioCode.Suzuki.Advanced.pas
// CONTENTS       : Suzuki Radio Code Calculator - Advanced Multi-Variant
// VERSION        : 2.0
...
//------------------------------------------------------------------------------
unit OBD.RadioCode.Suzuki.Advanced;
...
type
  /// <summary>
  ///   Suzuki Advanced Radio Code Calculator with Multiple Algorithm Variants
  /// </summary>
  TOBDRadioCodeSuzukiAdvanced = class(TOBDRadioCode)
  private
...
constructor TOBDRadioCodeSuzukiAdvanced.Create;
begin
  inherited Create;
  FVariantManager := TRadioCodeVariantManager.Create('Suzuki Advanced');
  InitializeVariants;
  FCurrentVariant := FVariantManager.GetDefaultVariant;
end;

destructor TOBDRadioCodeSuzukiAdvanced.Destroy;
begin
  FVariantManager.Free;
  inherited Destroy;
end;

procedure TOBDRadioCodeSuzukiAdvanced.InitializeVariants;
var
...
begin
  ...
  Code[2] := ApplyModularTransform(D3 * 8 + D4 + 1, 10);
  Code[3] := ApplyModularTransform(D4 * 2 + D1 + 9, 10);

  Output := Format('%d%d%d%d', [Code[0], Code[1], Code[2], Code[3]]);
end;

function TOBDRadioCodeSuzukiAdvanced.CalculateV2(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeSuzukiAdvanced.CalculateV3(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeSuzukiAdvanced.GetDescription: string;
begin
  Result := 'Advanced Suzuki Radio Code Calculator with multiple algorithm variants';
end;

procedure TOBDRadioCodeSuzukiAdvanced.SetVariant(const VariantID: string);
var
  Variant: TRadioCodeVariant;
begin
...
procedure TOBDRadioCodeSuzukiAdvanced.SetVariant(const Region: TRadioCodeRegion; const ModelYear: Integer);
var
  Variant: TRadioCodeVariant;
begin
...
function TOBDRadioCodeSuzukiAdvanced.GetCurrentVariant: TRadioCodeVariant;
begin
  Result := FCurrentVariant;
end;

function TOBDRadioCodeSuzukiAdvanced.GetAvailableVariants: TRadioCodeVariantManager;
begin
  Result := FVariantManager;
end;

function TOBDRadioCodeSuzukiAdvanced.Validate(const Input: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
...
function TOBDRadioCodeSuzukiAdvanced.Calculate(const Input: string; var Output: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
  ...
  if (FCurrentVariant.VariantID = 'SUZUKI_EU_V1') or
     (FCurrentVariant.VariantID = 'SUZUKI_NA') then
    Output := CalculateV1(Sanitized)
  else if (FCurrentVariant.VariantID = 'SUZUKI_EU_V2') or
          (FCurrentVariant.VariantID = 'SUZUKI_ASIA') then
    Output := CalculateV2(Sanitized)
  else if FCurrentVariant.VariantID = 'SUZUKI_EU_V3' then
