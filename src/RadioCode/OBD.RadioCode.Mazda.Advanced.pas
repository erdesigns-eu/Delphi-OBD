...
// CONTENTS       : Mazda Radio Code Calculator - Advanced Multi-Variant
// VERSION        : 2.0
...
type
  /// <summary>
  ///   Mazda Advanced Radio Code Calculator with Multiple Algorithm Variants
  /// </summary>
...
begin
  inherited Create;
  FVariantManager := TRadioCodeVariantManager.Create('Mazda Advanced');
  InitializeVariants;
...
function TOBDRadioCodeMazdaAdvanced.CalculateV1(const Serial: string): string;
var
  Sanitized: string;
  Serial: Integer;
...
function TOBDRadioCodeMazdaAdvanced.CalculateV2(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeMazdaAdvanced.CalculateV3(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
begin
  Result := 'Advanced Mazda Radio Code Calculator with multiple algorithm variants';
end;
...
function TOBDRadioCodeMazdaAdvanced.GetAvailableVariants: TRadioCodeVariantManager;
begin
...
begin
  ...
  if (FCurrentVariant.VariantID = 'MAZDA_EU_V1') or
     (FCurrentVariant.VariantID = 'MAZDA_NA') then
    Output := CalculateV1(Sanitized)
  else if (FCurrentVariant.VariantID = 'MAZDA_EU_V2') or
          (FCurrentVariant.VariantID = 'MAZDA_ASIA') then
    Output := CalculateV2(Sanitized)
  else if FCurrentVariant.VariantID = 'MAZDA_EU_V3' then
