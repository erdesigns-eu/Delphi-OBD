//------------------------------------------------------------------------------
// UNIT           : OBD.RadioCode.Infiniti.Advanced.pas
// CONTENTS       : Infiniti Radio Code Calculator - Advanced Multi-Variant
// VERSION        : 2.0
...
//------------------------------------------------------------------------------
unit OBD.RadioCode.Infiniti.Advanced;
...
type
  /// <summary>
  ///   Infiniti Advanced Radio Code Calculator with Multiple Algorithm Variants
  /// </summary>
  TOBDRadioCodeInfinitiAdvanced = class(TOBDRadioCode)
  private
...
constructor TOBDRadioCodeInfinitiAdvanced.Create;
begin
  inherited Create;
  FVariantManager := TRadioCodeVariantManager.Create('Infiniti Advanced');
  InitializeVariants;
  FCurrentVariant := FVariantManager.GetDefaultVariant;
end;

destructor TOBDRadioCodeInfinitiAdvanced.Destroy;
begin
  FVariantManager.Free;
  inherited Destroy;
end;

procedure TOBDRadioCodeInfinitiAdvanced.InitializeVariants;
var
...
function TOBDRadioCodeInfinitiAdvanced.CalculateV1(const Serial: string): string;
var
  Sanitized: string;
  Serial: Integer;
...
function TOBDRadioCodeInfinitiAdvanced.CalculateV2(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeInfinitiAdvanced.CalculateV3(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeInfinitiAdvanced.GetDescription: string;
begin
  Result := 'Advanced Infiniti Radio Code Calculator with multiple algorithm variants';
end;

procedure TOBDRadioCodeInfinitiAdvanced.SetVariant(const VariantID: string);
var
  Variant: TRadioCodeVariant;
begin
...
procedure TOBDRadioCodeInfinitiAdvanced.SetVariant(const Region: TRadioCodeRegion; const ModelYear: Integer);
var
  Variant: TRadioCodeVariant;
begin
...
function TOBDRadioCodeInfinitiAdvanced.GetCurrentVariant: TRadioCodeVariant;
begin
  Result := FCurrentVariant;
end;

function TOBDRadioCodeInfinitiAdvanced.GetAvailableVariants: TRadioCodeVariantManager;
begin
  Result := FVariantManager;
end;

function TOBDRadioCodeInfinitiAdvanced.Validate(const Input: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
...
function TOBDRadioCodeInfinitiAdvanced.Calculate(const Input: string; var Output: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
  ...
  if (FCurrentVariant.VariantID = 'INFINITI_EU_V1') or
     (FCurrentVariant.VariantID = 'INFINITI_NA') then
    Output := CalculateV1(Sanitized)
  else if (FCurrentVariant.VariantID = 'INFINITI_EU_V2') or
          (FCurrentVariant.VariantID = 'INFINITI_ASIA') then
    Output := CalculateV2(Sanitized)
  else if FCurrentVariant.VariantID = 'INFINITI_EU_V3' then
