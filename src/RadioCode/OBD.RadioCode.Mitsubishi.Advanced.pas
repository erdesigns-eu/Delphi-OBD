//------------------------------------------------------------------------------
// UNIT           : OBD.RadioCode.Mitsubishi.Advanced.pas
// CONTENTS       : Mitsubishi Radio Code Calculator - Advanced Multi-Variant
// VERSION        : 2.0
...
//------------------------------------------------------------------------------
unit OBD.RadioCode.Mitsubishi.Advanced;
...
type
  /// <summary>
  ///   Mitsubishi Advanced Radio Code Calculator with Multiple Algorithm Variants
  /// </summary>
  TOBDRadioCodeMitsubishiAdvanced = class(TOBDRadioCode)
  private
...
constructor TOBDRadioCodeMitsubishiAdvanced.Create;
begin
  inherited Create;
  FVariantManager := TRadioCodeVariantManager.Create('Mitsubishi Advanced');
  InitializeVariants;
  FCurrentVariant := FVariantManager.GetDefaultVariant;
end;

destructor TOBDRadioCodeMitsubishiAdvanced.Destroy;
begin
  FVariantManager.Free;
  inherited Destroy;
end;

procedure TOBDRadioCodeMitsubishiAdvanced.InitializeVariants;
var
...
begin
  ...
  Variant := FVariantManager.AddVariant(
    'MITSUBISHI_EU_V1',
    'Mitsubishi Europe V1 (1995-2005)',
    rcrEurope,
    1995, 2005,
    rcsvV1,
    True
  );
  Variant.RadioModels.Add('Rockford');
  Variant.AlgorithmNotes := 'Basic algorithm for European market';
  
  // Europe V2 (2006-2015)
  Variant := FVariantManager.AddVariant(
    'MITSUBISHI_EU_V2',
    'Mitsubishi Europe V2 (2006-2015)',
    rcrEurope,
    2006, 2015,
    rcsvV2
  );
...
function TOBDRadioCodeMitsubishiAdvanced.CalculateV1(const Serial: string): string;
var
  Sanitized: string;
  D1, D2, D3, D4: Integer;
...
begin
  ...
  Code[0] := ApplyModularTransform(D1 * 5 + D3 + 8, 10);
  Code[1] := ApplyModularTransform(D2 * 7 + D4 + 2, 10);
  Code[2] := ApplyModularTransform(D3 * 3 + D1 + 6, 10);
  Code[3] := ApplyModularTransform(D4 * 9 + D2 + 4, 10);

  Output := Format('%d%d%d%d', [Code[0], Code[1], Code[2], Code[3]]);
end;

function TOBDRadioCodeMitsubishiAdvanced.CalculateV2(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeMitsubishiAdvanced.CalculateV3(const Serial: string): string;
var
  SerialNum: Integer;
  Code: array[0..3] of Integer;
begin
...
function TOBDRadioCodeMitsubishiAdvanced.GetDescription: string;
begin
  Result := 'Advanced Mitsubishi Radio Code Calculator with multiple algorithm variants';
end;

procedure TOBDRadioCodeMitsubishiAdvanced.SetVariant(const VariantID: string);
var
  Variant: TRadioCodeVariant;
begin
...
procedure TOBDRadioCodeMitsubishiAdvanced.SetVariant(const Region: TRadioCodeRegion; const ModelYear: Integer);
var
  Variant: TRadioCodeVariant;
begin
...
function TOBDRadioCodeMitsubishiAdvanced.GetCurrentVariant: TRadioCodeVariant;
begin
  Result := FCurrentVariant;
end;

function TOBDRadioCodeMitsubishiAdvanced.GetAvailableVariants: TRadioCodeVariantManager;
begin
  Result := FVariantManager;
end;

function TOBDRadioCodeMitsubishiAdvanced.Validate(const Input: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
...
function TOBDRadioCodeMitsubishiAdvanced.Calculate(const Input: string; var Output: string; var ErrorMessage: string): Boolean;
var
  Sanitized: string;
begin
  Result := True;
  ...
  if (FCurrentVariant.VariantID = 'MITSUBISHI_EU_V1') or
     (FCurrentVariant.VariantID = 'MITSUBISHI_NA') then
    Output := CalculateV1(Sanitized)
  else if (FCurrentVariant.VariantID = 'MITSUBISHI_EU_V2') or
          (FCurrentVariant.VariantID = 'MITSUBISHI_ASIA') then
    Output := CalculateV2(Sanitized)
  else if FCurrentVariant.VariantID = 'MITSUBISHI_EU_V3' then
